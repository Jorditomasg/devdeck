...
- Stopping a WSL service now truly kills its Linux process tree. The in-distro
  kill had never worked on Ubuntu (`/bin/sh` is dash, which rejects the kill
  syntax the app used), so every stop silently left the tree — and its port —
  alive until a manual `wsl --shutdown`. This was the root cause of restarts
  failing with "Port already in use".
- Dev servers started by Nx are killed too: Nx runs them in their own process
  session (pseudo-terminal), outside the group previous kills targeted. Stops
  and the crash-safety lifeline now walk the whole descendant tree.
...
- Branch and stash tables in the git window now have a select-all checkbox,
  and the bulk delete button is always visible.
- The tray panel now lists repositories in the same order as the main window
  (your drag-and-drop order, alphabetical otherwise).

### Fixed

- Restarting a service now waits for its port to be released between stop and
  start, preventing failed restarts on slow-to-release ports.
