...
- Processes spawn in their OWN process group (Unix `process_group(0)`) or their OWN Job Object (Windows, `JOB_OBJECT_LIMIT_KILL_ON_JOB_CLOSE`, assigned right after spawn — see `src-tauri/src/process/job.rs`); stop escalates stop_cmd → Terminate (10 s) → ForceKill (5 s), with Windows Terminate/ForceKill routed through `TerminateJobObject` (falling back to `taskkill /F /T` if job assignment failed). This escalation is deliberate (kills whole process trees reliably, and the Job Object also guarantees cleanup on a DevDeck crash) — do not "simplify" it.
...
- Git auto-routes per repo path (Windows only): a repo under `\\wsl.localhost\<distro>\...` (or `\\wsl$`) runs git INSIDE the distro via `wsl.exe -d <distro> --cd <path> --exec git` (`git/exec.rs`); `--exec` is mandatory (no shell → no injection). Git READS additionally go through a persistent per-distro bash session (`git/session.rs`, near-native latency). Everything else uses Windows git unchanged.
- WSL services (repo under `\\wsl.localhost`) run in-distro via `wsl.exe … setsid -w bash -ilc <script>` (`wsl.rs`): the script exports env, emits a `__DEVDECK_PID__` PGID marker, and parks a **lifeline watchdog** on a piped stdin DevDeck never writes to — dropping the pipe (stop force step, app exit, deregistration, or ANY DevDeck death incl. crash) SIGKILLs the whole Linux group from inside the distro (design doc `docs/superpowers/specs/2026-07-17-wsl-lifeline-design.md`). `shutdown_all` fast-paths WSL runs without a `stop_cmd` through the lifeline (bounded by `LIFELINE_EXIT_WAIT`); do NOT re-route their exit through the full escalation ladder.
