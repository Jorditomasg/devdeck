...
@Component({
  ...
  template: `
    ...
    <footer class="page__statusbar">
      ...
      @if (contextText()) {
        <span class="page__context">{{ contextText() }}</span>
      }
...
export class WorkspacePageComponent {
  ...
  /** Active group + profile at a glance ('' hides the segment). */
  protected readonly contextText = computed(() => {
    const parts: string[] = [];
    const group = this.settings.activeGroup()?.name;
    if (group) {
      parts.push(`${this.i18n.t('label.group')} ${group}`);
    }
    const profile = this.profiles.activeProfileName();
    if (profile) {
      parts.push(`${this.i18n.t('label.profile')} ${profile}`);
    }
    return parts.join(' · ');
  });
   ...
   *  itself (and its card-collapse) lives in WorkspaceStore so cards can read it. */
  protected readonly dragEnabled = computed(
    () => this.ws.reorderMode() && !this.ws.repoFilter().trim(),
  );
